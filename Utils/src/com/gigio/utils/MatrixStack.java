...
public class MatrixStack
{
	...
	{
		this.mMatrix = new float[maxDepth * MATRIX_SIZE];
		this.mTemp = new float[MATRIX_SIZE * 2];
		glLoadIdentity();
	}
	...
	{
		Matrix.frustumM(this.mMatrix, this.mTop, left, right, bottom, top,
				near, far);
	}
	...
	{
		System.arraycopy(m, offset, this.mMatrix, this.mTop, MATRIX_SIZE);
	}

	public void glLoadMatrixf(FloatBuffer m)
	{
		m.get(this.mMatrix, this.mTop, MATRIX_SIZE);
	}
	...
	{
		m.get(this.mTemp, MATRIX_SIZE, MATRIX_SIZE);
	...
	public void glMultMatrixx(int[] m, int offset)
	{
		for (int i = 0; i < MATRIX_SIZE; i++)
		{
			this.mTemp[MATRIX_SIZE + i] = fixedToFloat(m[offset + i]);
		}
		glMultMatrixf(this.mTemp, MATRIX_SIZE);
	}
	...
	public void glMultMatrixx(IntBuffer m)
	{
		for (int i = 0; i < MATRIX_SIZE; i++)
		{
			this.mTemp[MATRIX_SIZE + i] = fixedToFloat(m.get());
		}
		glMultMatrixf(this.mTemp, MATRIX_SIZE);
	}
	...
	public void glOrthof(float left, float right, float bottom, float top,
			float near, float far)
	{
		Matrix.orthoM(this.mMatrix, this.mTop, left, right, bottom, top, near,
				far);
	}
	...
	{
		preflight_adjust(1);
		System.arraycopy(this.mMatrix, this.mTop, this.mMatrix, this.mTop
	...
	public void glRotatef(float angle, float x, float y, float z)
	{
		...
		System.arraycopy(this.mMatrix, this.mTop, this.mTemp, MATRIX_SIZE,
				MATRIX_SIZE);
		Matrix.multiplyMM(this.mMatrix, this.mTop, this.mTemp, MATRIX_SIZE,
				this.mTemp, 0);
	}
	...
	{
		System.arraycopy(this.mMatrix, this.mTop, dest, offset, MATRIX_SIZE);
	}
	...
	{
		final int newTop = this.mTop + dir * MATRIX_SIZE;
		if (newTop < 0)
		{
			throw new IllegalArgumentException("stack underflow");
		}
		if (newTop + MATRIX_SIZE > this.mMatrix.length)
		{
